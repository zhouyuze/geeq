...
compare <- function(cluster.size) {
  ...
  for (i in 1:group) {
    ...
    start <- Sys.time()
    geeq(count~time, data=testdat, id=subject, method='gee', family=poisson, corstr="ar1")
    end <- Sys.time()
    result[i, 2] <- as.double(end - start)

    start <- Sys.time()
    geeq(count~time, data=testdat, id=subject, method='qif', family=poisson, corstr="ar1")
    end <- Sys.time()
