...
correlation_select <- function(n, repeat_time) {
  ...
  for (i in 1:repeat_time) {
    ...
    qic[1] <- geeq(y~x1+x2, data=data, id=id, method='gee', family=binomial, corstr="independence")$QIC
    qic[2] <- geeq(y~x1+x2, data=data, id=id, method='gee', family=binomial, corstr="exchangeable")$QIC
    qic[3] <- geeq(y~x1+x2, data=data, id=id, method='gee', family=binomial, corstr="ar1")$QIC
    index <- which.min(qic)
...
variable_select <- function(n, repeat_time, cor_str) {
  ...
  for (i in 1:repeat_time) {
    data <- generatedata(n, 3)
    qic[1] <- geeq(y~x1, data=data, id=id, method='gee', family=binomial, corstr=cor_str)$QIC
    qic[2] <- geeq(y~x1+x2, data=data, id=id, method='gee', family=binomial, corstr=cor_str)$QIC
    qic[3] <- geeq(y~x1+x3, data=data, id=id, method='gee', family=binomial, corstr=cor_str)$QIC
    qic[4] <- geeq(y~x1+x2+x3, data=data, id=id, method='gee', family=binomial, corstr=cor_str)$QIC
    qic[5] <- geeq(y~x1+x2+x3+x4, data=data, id=id, method='gee', family=binomial, corstr=cor_str)$QIC
    index <- which.min(qic)
    select[1, index] <- select[1, index] + 1
  }
